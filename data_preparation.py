...
def load_and_process_data(data_path, window_size=300, normal_beats_limit=10000):
    heartbeats = []
    labels = []
    normal_beats_count = 0
        ...
        for i in range(len(annotation.symbol)):
            ...
            if symbol in ['N', 'L', 'R', 'A', 'a', 'J', 'S', 'V', 'E', 'F', '/']:
                ...
                if len(heartbeat) == window_size:
                    if label == 0 and normal_beats_count < normal_beats_limit:
                        heartbeats.append(heartbeat)
                        labels.append(label)
                        normal_beats_count += 1
                    elif label != 0:
                        heartbeats.append(heartbeat)
                        labels.append(label)
